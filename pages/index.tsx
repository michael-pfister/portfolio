		...
		<section className="flex justify-evenly my-16" id="my_advantage">
			...
			<div className="flex flex-col gap-8 items-center">
				<h1 className="text-3xl">
					My <text className="text-orange">Advantage</text>
				</h1>
			...
			<div className="flex justify-evenly items-center">
				<div>
					<h1 className="text-3xl p-4">
						Latest <text className="text-blue">Projects</text>
					</h1>
					<div className="p-4">
						<Seperator />
...
const TwitterFeed = ({ tweets }: { tweets: TwitterTweet[] }) => {
	return (
		<section className="my-8 px-4" id="twitter_feed">
			<h1 className="text-4xl w-full text-center mb-8">
				Recent <text className="text-[#1d9bf0]">Tweets</text>
			</h1>
			<div className="flex flex-wrap gap-x-5 justify-center items-center">
				{tweets.map(({ id }, index) => (
					<div
						key={`twitter_tweet_${id}`}
						className={`${index > 1 && "hidden sm:inline-block"}`}
						data-aos="fade-up"
						data-cy="twitter tweet"
					>
...
const ContactMe = () => {
	return (
		<section className=" bg-orange" id="contact_me">
			<div
				className="w-full h-16 sm:h-28 md:h-36 lg:h-52 xl:h-72 bg-[url('/wave-haikei.svg')] bg-cover bg-bottom"
				aria-hidden="true"
			/>
			<div className="p-4 flex justify-center items-center flex-col sm:flex-row gap-4">
				<div className="flex flex-col items-center" data-aos="fade-right">
					...
					<div className="relative w-64 md:w-96 lg:w-[500px] aspect-[406/306] hidden sm:inline-block">
						<Image
							src="/Sending emails_Flatline.svg"
							alt="Contact me"
							layout="fill"
						/>
					</div>
				...
				<form
					className="flex flex-col gap-4 w-64 md:w-[500px]"
					data-aos="fade-left"
					data-cy="contact form"
					action="https://formsubmit.co/8b0ed63f53eab46acdfa37976cb768dc"
					method="POST"
				>
					<input
						type="text"
...
const Home: NextPage<PageProps> = ({ githubRepositories, twitterTweets }) => {
	return (
		<>
			...
			<MyAdvantages />
			{githubRepositories && (
				<LatestProjects githubRepositories={githubRepositories} />
			)}
			{twitterTweets && <TwitterFeed tweets={twitterTweets} />}
			<section className="my-16 flex justify-center p-2" id="twitter_feed">
				<p className="text-center">
					Thanks to Elon Musk I cannot display my Tweets on a free basis
					anymore. Please{" "}
					<a
